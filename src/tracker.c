...
void tracker_init(void)
{
	...
	LOG_INF("Tracker Initialized with %d entries, PSRAM payload 0x%06x-0x%06x (%d bytes, slot size=%d)",
...
void tracker_default_expired_cb(int index, struct data_tracker *entry, bool exhausted)
{
	if (exhausted) {
		...
		if (entry->packet_type == PACKET_PING_DEVICE && entry->dst_id == get_temp_id()) {
			LOG_WRN("Ping tracker for temp_id: %d exhausted, clearing temp_id", get_temp_id());
			set_temp_id(0xFFFF);
		}

		if (entry->packet_type == PACKET_JOINED_NETWORK || entry->packet_type == PACKET_CONFIG) {
	...
	if (entry->payload_len > 0) {
		...
		LOG_WRN("No payload for packet type 0x%02x retry, cannot resend", entry->packet_type);
	}
}
