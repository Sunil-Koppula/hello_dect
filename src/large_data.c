...
static uint8_t chunk_size_for_large_data(uint8_t chunk_idx, uint8_t sender_idx)
{
...
static int send_next_large_data_chunk(uint16_t dst_id, uint8_t dst_type, uint8_t sender_idx)
{
    ...
    uint8_t csz = chunk_size_for_large_data(ld_sender[sender_idx].next_chunk, sender_idx);
...
static uint8_t validate_large_data_chunk(const large_data_chunk_t *pkt)
{
    ...
    if (pkt->page_index >= ld_slot[idx].page_count || pkt->chunk_index > 20 || (pkt->page_index * 20 + pkt->chunk_index) > ld_slot[idx].total_chunks) {
        LOG_WRN("LARGE_DATA_CHUNK rejected: Page: %d Chunk: %d out of bounds for gen %d data_id %d (page_count: %d, total_chunks: %d)", pkt->page_index, pkt->chunk_index, pkt->gen_device_id, pkt->data_id, ld_slot[idx].page_count, ld_slot[idx].total_chunks);
    ...
    uint32_t addr = ld_slot[idx].base_addr + (uint32_t)(pkt->page_index * 20 + pkt->chunk_index) * SEND_DATA_MAX;
    uint8_t csz = (pkt->page_index * 20 + pkt->chunk_index) == ld_slot[idx].total_chunks - 1 ? ld_slot[idx].last_chunk_size : SEND_DATA_MAX;
...
static void anchor_large_data_tick(void)
{
    ...
    for (int sender_idx = 0; sender_idx < 4; sender_idx++) {
        ...
        nbtimeout_init(&ld_sender[sender_idx].timeout, SENDER_TIMEOUT_MS, 0);
    ...
    switch (get_device_type()) {
        ...
        case DEVICE_TYPE_SENSOR:
        {
            if (pkt->hdr.device_type == DEVICE_TYPE_GATEWAY || pkt->hdr.device_type == DEVICE_TYPE_ANCHOR) {
                uint8_t sender_idx = find_sender_slot(dst_id, pkt->data_id);
                if (sender_idx < 0) {
...
void cmd_ld_init(const char *args)
{
    ...
    int idx = find_large_data_slot(get_serial_number(), (uint16_t)v_id);
    if (idx < 0) {
    ...
    ld_slot[idx].data_id = (uint16_t)v_id;
    ld_slot[idx].total_size = (uint32_t)v_total;
    ld_slot[idx].total_chunks = (uint16_t)v_chunks;
    ld_slot[idx].last_chunk_size = (uint16_t)v_last;
    ld_slot[idx].crc32 = (uint32_t)v_crc32;

    LOG_INF("AT#LDINIT accepted: sn=0x%016llx id=%u total=%u chunks=%u last=%u crc32=0x%08x",
